...
export function Navbar() {
    ...
    return (<nav className="flex justify-between items-center p-4 bg-white shadow">
...
export function DestinationsPage() {
    ...
    return (<div className="grid grid-cols-3 gap-4 p-8">
      {destinations.map(dest => (<div key={dest._id} className="border rounded overflow-hidden shadow">
...
export function BookingForm() {
    ...
    return (<form onSubmit={handleSubmit} className="max-w-md mx-auto p-6 border rounded">
      <h2 className="text-2xl font-bold mb-4">New Booking</h2>

      {message && (<div className={`p-3 rounded mb-4 ${message.type === 'success'
      ...
      <div className="mb-4">
        ...
        <input type="text" name="destination" value={formData.destination} onChange={handleChange} disabled={loading} required className="w-full border rounded p-2"/>
      </div>

      <div className="mb-4">
        <label className="block font-semibold mb-2">Date</label>
        <input type="date" name="date" value={formData.date} onChange={handleChange} disabled={loading} required className="w-full border rounded p-2"/>
      </div>

      <div className="mb-4">
        <label className="block font-semibold mb-2">Number of Guests</label>
        <input type="number" name="guests" value={formData.guests} onChange={handleChange} disabled={loading} min="1" className="w-full border rounded p-2"/>
      </div>

      <button type="submit" disabled={loading} className="w-full bg-blue-500 text-white py-2 rounded font-semibold hover:bg-blue-600 disabled:bg-gray-400">
...
export function ProfilePage() {
    ...
    return (<div className="p-8 max-w-2xl mx-auto">
      ...
      <div className="bg-gray-100 p-6 rounded">
        <img src={user?.avatar || 'https://via.placeholder.com/100'} alt={user?.name} className="w-24 h-24 rounded-full mb-4"/>
...
export function ValidatedForm() {
    ...
    return (<div className="max-w-md">
      <input type="email" value={email} onChange={handleChange} className={`w-full p-2 border rounded ${error ? 'border-red-500' : 'border-gray-300'}`} placeholder="Enter email"/>
